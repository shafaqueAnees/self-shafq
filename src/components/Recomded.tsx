...
const CategorySection: React.FC<{ title: string; items: any[] }> = ({ title, items }) => {
  ...
  return (
    <div className="py-10 px-4 sm:px-6 lg:px-10">
      <h2 className="text-2xl font-semibold mb-6 text-center sm:text-left">{title}</h2>
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
        {items.slice(0, visibleItems).map((item) => (
          <div
            key={item.id}
            className="flex flex-col justify-between bg-white rounded-lg shadow-md p-5 hover:shadow-lg transition-shadow"
          >
            {/* Item Image */}
            <img
              src={item.image}
...
const RecomendedCar: React.FC = () => {
  ...
  return <CategorySection title="Recommended Cars" items={recommendedCars} />;
};
